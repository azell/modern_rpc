    ...
    StreamingGreeterGrpc.StreamingGreeterImplBase svc =
        new StreamingGreeterGrpc.StreamingGreeterImplBase() {
          ...
          public StreamObserver<HelloRequest> sayHelloStreaming(
            ...
            // Set up manual flow control for the request stream. It feels backwards to configure
            // the request
            // stream's flow control using the response stream's observer, but this is the way it
            // is.
            ...
            // Set up a back-pressure-aware consumer for the request stream. The onReadyHandler will
            // be invoked
            // when the consuming side has enough buffer space to receive more messages.
            //
            // Note: the onReadyHandler's invocation is serialized on the same thread pool as the
            // incoming StreamObserver's
            // onNext(), onError(), and onComplete() handlers. Blocking the onReadyHandler will
            // prevent additional messages
            // from being processed by the incoming StreamObserver. The onReadyHandler must return
            // in a timely manor or else
            ...
            return new StreamObserver<HelloRequest>() {
              ...
              public void onNext(HelloRequest request) {
                ...
                try {
                  ...
                  // Check the provided ServerCallStreamObserver to see if it is still ready to
                  // accept more messages.
                  if (serverCallStreamObserver.isReady()) {
                    // Signal the sender to send another request. As long as isReady() stays true,
                    // the server will keep
                    // cycling through the loop of onNext() -> request()...onNext() -> request()...
                    // until either the client
                    // runs out of messages and ends the loop or the server runs out of receive
                    // buffer space.
                    //
                    // If the server runs out of buffer space, isReady() will turn false. When the
                    // receive buffer has
                    // sufficiently drained, isReady() will turn true, and the
                    // serverCallStreamObserver's onReadyHandler
