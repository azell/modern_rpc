...
public class ManualFlowControlClient {
  ...
  public static void main(String[] args) throws InterruptedException {
    ...
    // When using manual flow-control and back-pressure on the client, the ClientResponseObserver
    // handles both
    ...
    ClientResponseObserver<HelloRequest, HelloReply> clientResponseObserver =
        new ClientResponseObserver<HelloRequest, HelloReply>() {
          ...
          public void beforeStart(final ClientCallStreamObserver<HelloRequest> requestStream) {
            ...
            // Set up manual flow control for the response stream. It feels backwards to configure
            // the response
            // stream's flow control using the request stream's observer, but this is the way it is.
            requestStream.disableAutoInboundFlowControl();

            // Set up a back-pressure-aware producer for the request stream. The onReadyHandler will
            // be invoked
            // when the consuming side has enough buffer space to receive more messages.
            //
            // Messages are serialized into a transport-specific transmit buffer. Depending on the
            // size of this buffer,
            // MANY messages may be buffered, however, they haven't yet been sent to the server. The
            // server must call
            ...
            // incoming
            // StreamObserver'sonNext(), onError(), and onComplete() handlers. Blocking the
            // onReadyHandler will prevent
            // additional messages from being processed by the incoming StreamObserver. The
            // onReadyHandler must return
